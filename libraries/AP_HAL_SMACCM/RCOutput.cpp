...
static uint32_t last_motor_print;
...
static void update_motors(uint32_t now)
{
    ...
    if (now - last_motor_print > 100000) { // 100ms print rate
        hal.console->printf("motors: %u %u %u %u\r\n",
                            periods[0], periods[1],
                            periods[2], periods[3]);
        last_motor_print = now;
    }
}
...
{
    if (ch < 4) {
        if (period_us != periods[ch]) {
            //hal.console->printf("RCO: ch %u  period %u\r\n", ch, period_us);
